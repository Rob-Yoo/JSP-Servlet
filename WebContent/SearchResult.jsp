...
<body>
		...
		<div class="p-1 mt-3 ml-3 bg-dark text-white text-center rounded-pill" style="max-width: 20%">
			<h3 class="mt-1 mb-0">대여 도서 평가</h3>	
		</div>
		...
		<c:forEach var="cnt" begin="0" end="${SearchList.listSize - 1}" varStatus="status">
			<c:if test ="${not doneLoop}">
	    		...
	    		<div class="card bg-light mt-3">
	    			<div class="card-header bg-dark text-white">
		    			<div class="row">
		    				<div class="col-8 text-left">&nbsp;${SearchList.bookName[cnt]}&nbsp;<small>${SearchList.authorName[cnt]}</small></div>
		    				<div class="col-4 text-right">종합&nbsp;
		    				<c:set var="total" value="${SearchList.totalScore[cnt]}" />
		    				<c:if test="${total == '매우 나쁨' or total == '나쁨'}">
		    					<span class="text-danger"><i>${SearchList.totalScore[cnt]}</i></span>
		    				</c:if>
		    				<c:if test="${total == '보통'}">
		    					<span class="text-success"><i>${SearchList.totalScore[cnt]}</i></span>
		    				</c:if>
		    				<c:if test="${total == '매우 좋음' or total == '좋음'}">
		    					<span class="text-primary"><i>${SearchList.totalScore[cnt]}</i></span>
		    				</c:if>
		    				</div>
		    			</div>
		    		</div>
	    		<div class="card-body">
	    			<h5 class="card-title">${SearchList.evaluationTitle[cnt]}</h5>
	    			<p class="card-text">${SearchList.evaluationContent[cnt]}</p>
	    		</div>
	    		<div class="card-footer">
	    			<div class = "row">
	    				<c:set var="ID" value="${SearchList.userID[cnt]}" />
	    				<c:set var="id" value="${userID}" />
	    				<c:if test="${ID == id}">
	    					<div class="col-9 text-left mt-2">
		    					책 수준:&nbsp;<span style="color: #3c40c6;">${SearchList.standard[cnt]}</span>
		    					|&nbsp;책 종류:&nbsp;<span style="color: #3c40c6;">${SearchList.content[cnt]}</span>
		    					|&nbsp;책 상태:&nbsp;<span style="color: #3c40c6;">${SearchList.bookCondition[cnt]}</span>
	    					</div>
	    				</c:if>	    				
	    				<c:if test="${ID != id}">
	    				...
	    				<c:if test="${ID == id}">
		    				<div class="col-3 text-right">
		    					<form action="deleteAction?evaluationID=${SearchList.evaluationID[cnt]}" method ="post">
		    						<a class="btn btn-info" data-toggle="modal" href="#registerModal${SearchList.evaluationID[cnt]}">수정</a>
		    						<button type="submit" class="btn btn-danger">삭제</button>
		    					</form>
	    					</div>
	    					<div class="modal fade" id="registerModal${SearchList.evaluationID[cnt]}" tabindex="-1" role="dialog" aria-labelledby="modal" aria-hidden="true">
						    	<div class="modal-dialog">
						    		<div class="modal-content">
						    			...
						    			<div class="modal-body">
						    				<form action="modify?evaluationID=${SearchList.evaluationID[cnt]}" method="post">
						    					<div class="form-row"> 
						    						<div class="form-group col-sm-6"> 
						    							<label>책 제목</label>
						    							<input type="text" name="bookName" class="form-control" maxlength="20" value="${SearchList.bookName[cnt]}">
						    						</div>
						    						<div class="form-group col-sm-6">
						    							<label>저자명</label>
						    							<input type="text" name="authorName" class="form-control" maxlength="20" value="${SearchList.authorName[cnt]}">
						    						</div>
						    					</div>
						    					<div class="form-group">
						    						<label>제목</label>
						    						<input type="text" name="evaluationTitle" class="form-control" maxlength="30" value="${SearchList.evaluationTitle[cnt]}">
						    					</div>
						    					<div class="form-group">
						    						<label>내용</label>
						    						<textarea name="evaluationContent" class="form-control" maxlength="2048" style="height: 180px;">${SearchList.evaluationContent[cnt]}</textarea>
						    					</div>
						    					<div class="form-row">
	    ...
	    <ul class= "pagination justify-content-center mt-3">
	    	...
	    	<li>
				<c:set var="size" value="${SearchList.listSize}" />
				<c:if test="${size < 6}">
	    ...
	    <div class="modal fade" id="registerModal2" tabindex="-1" role="dialog" aria-labelledby="modal" aria-hidden="true">
	    	<div class="modal-dialog">
	    		<div class="modal-content">
	    			<div class="modal-header">
	    				<h5 class="modal-title" id="modal">평가 등록</h5> 
